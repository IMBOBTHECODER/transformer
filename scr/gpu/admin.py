            ...
            if (step + 1) % cfg.val_interval == 0:
                ...
                with torch.no_grad():
                    for x_vbatch, y_vbatch in self.val_loader:
                        ...
                        x_vbatch = x_vbatch.to(device=device, dtype=torch.long)
                        y_vbatch = y_vbatch.to(device=device, dtype=torch.long)
...
if __name__ == "__main__":
    ...
    # Set resume=True to continue training from the saved checkpoint
    # Or use a custom checkpoint path: control.train(resume=True, checkpoint_path="path/to/checkpoint.pt")
    control.train(resume=False)  # Change to resume=True to load previous model