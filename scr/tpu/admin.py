...
def create_data_loaders(token_stream, cfg, val_split=0.05):
    ...
    # TPU: no workers, MpDeviceLoader handles parallelism
    ...
    # Wrap with MpDeviceLoader for XLA input pipeline fusion
...
def warmup_xla_graph(model, batch_size, seq_len, device):
    """Warm up XLA compilation for consistent training speed."""
...
class Control:
    ...
    def train(self, resume=False, checkpoint_path=None):
        ...
        # Learning rate schedule (precomputed to avoid device syncs)
        ...
        while step < cfg.total_steps:
            ...
            # MpDeviceLoader already places on device
            ...
            # Gradient clipping (disabled by default on TPU)
            ...
            # Mark XLA step (critical for TPU)
            torch_xla.sync()
            ...
            if (step + 1) % cfg.val_interval == 0:
                # TPU: use torch.no_grad() instead of eval() to avoid XLA recompilation
                ...
                # Mark XLA step after validation
                torch_xla.sync()
